  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-40 bg-indigo-600 text-white p-4 rounded-full shadow-xl hover:bg-indigo-700 hover:scale-105 transition-all flex items-center justify-center ${isOpen ? 'scale-0 opacity-0' : 'scale-100 opacity-100'}`}
      ...
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-indigo-600 to-violet-600 text-white p-4 flex items-center justify-between shadow-sm z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-indigo-100 text-xs font-medium">AI-Powered Savings Assistant</p>
            </div>
          </div>
          <button 
            ...
            >
              <div className={`max-w-[85%] ${msg.role === 'user' ? 'order-2' : 'order-1'}`}>
                ...
                {msg.suggestedDeals && msg.suggestedDeals.length > 0 && (
                  <div className="mt-2 bg-white border border-indigo-100 rounded-2xl overflow-hidden shadow-sm">
                    <div className="bg-indigo-50/50 px-3 py-2.5 border-b border-indigo-100 flex items-center justify-between">
                      ...
                      <button 
                        ...
                        className="text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-white px-2.5 py-1 rounded-lg border border-indigo-200 shadow-sm flex items-center gap-1 transition-colors hover:shadow-md"
                      >
                        ...
                        return (
                          <div key={`${deal.product_id}-${idx}`} className="p-2.5 flex items-center justify-between gap-3 hover:bg-slate-50 transition-colors group">
                            <div className="flex-1 min-w-0">
                              <div className="text-sm font-bold text-slate-900 truncate group-hover:text-indigo-600 transition-colors" title={deal.name}>{deal.name}</div>
                            ...
                            <div className="text-right flex-shrink-0">
                              <div className="text-sm font-black text-indigo-600">${price.toFixed(2)}</div>
                            </div>
                            <button
                              ...
                              className={`ml-1 p-2 rounded-xl flex-shrink-0 transition-all ${
                                inList 
                                  ? 'bg-slate-100 text-indigo-600 cursor-default' 
        ...
        <div className="p-4 bg-white border-t border-slate-100">
          <div className="flex items-end gap-2 bg-slate-50 border border-slate-200 rounded-2xl p-1.5 focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500 transition-all shadow-sm">
            <textarea
              value={input}
            ...
            <button
              ...
              className={`p-3 rounded-xl flex-shrink-0 transition-all mb-0.5 mr-0.5 ${
                !input.trim() || isLoading 
                  ...
                  : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0'
              }`}
            >
